# Agentic Commentary (SQLite)

Purpose: text-only agentic commentary for the 2011 CWC final using `data/structured/sqlite/commentary.sqlite`.
...
- Step 2: Implemented SQLite loader in `Cri-CAN/agentic/sqlite/run_agentic_sqlite.py` to query the `balls` table in match order.
- Step 3: Wired the loader into the shared agentic pipeline and added CLI flags for input, limit, and output.
- Step 4: Generated a sample output file (`Cri-CAN/agentic/sqlite/sample_output.txt`) using the default limit.
- Step 5: Benchmarked load + full generation time and recorded results in `Cri-CAN/agentic/performance_report.md`.

## Tools used
- Python 3 standard library (`sqlite3`, `argparse`, `time`, `pathlib`).
...
```bash
python3 Cri-CAN/agentic/sqlite/run_agentic_sqlite.py
```

Custom:

```bash
python3 Cri-CAN/agentic/sqlite/run_agentic_sqlite.py --limit 200 --output Cri-CAN/agentic/sqlite/out.txt
```
...
## Outputs
- `Cri-CAN/agentic/sqlite/sample_output.txt` (first 60 balls by default)
...
- SQLite (this option): total_seconds = 0.0056
